...
include ($root_path."config/sungunura.php");
	...
	foreach ($_POST AS $key => $value)
	{
		if ($key!=='btnAnswer')
		{
			...
			while ($char_counter<$char_count)
			{
				...
				{
					$_SESSION['answer_validate']="Invalid characters entered<br>";
	...
	function answers_pool($cxn, $subject_code,$question_number,$a){
		$section_id=$_SESSION['section_counter'];
...
<body>
	...
	<div id="content" >
		...
		<?php
								...
								$count_query="SELECT MAX(answer_field) as total_expected_answers FROM tblStructuredAnswers WHERE 
													subject_code=$subject_code 
													AND topic_id IN (SELECT topic_id 
																	FROM tbltopics
																	WHERE section_id=$section_id)
								...
								if ($result=mysqli_query($cxn,$count_query)){
									...
									for($i=1;$i<=$total_expected_answers;$i++){
										...
										//$grade=$_SESSION['grade'];
										$question_counter=$_SESSION['question_counter'];
										$other_answers = answers_pool($cxn, $subject_code, $question_number,$i);
							...
							?>
							<hr/>
							<form  autocomplete="off" id="form1" name="form1" method="post" action="sq_dyn_questions.php">
